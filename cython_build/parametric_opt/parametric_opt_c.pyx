...
from utility.py_import import np, plt, cv2

cimport numpy as np
cimport cython
from libc.math cimport pow, atan
from src.cython.parametric_opt_c cimport ParameterOptimizer
...
cdef class ParameterOptimizer:
    ...
    def __init__(
        ...
        cdef:
            float[:] shape = self.parametric_X.shape
        ...
        self.rad[4] = min(shape[2], 1 - shape[2]) / 2
        self.rad[5] = 0.75 * shape[2]
        
        cdef int NRPos = int(np.ceil(self.num_interval / 2.0))
        self.n_rad[0] = NRPos
        self.n_rad[1] = NRPos
        ...
        if self.verbose:
            ...
            # Convert C arrays to NumPy arrays for printing
            cdef np.ndarray rad_np = np.array([self.rad[i] for i in range(6)])
            cdef np.ndarray n_rad_np = np.array([self.n_rad[i] for i in range(6)])
            print(f"#### Initial radius: {self.__format_verbose(rad_np)}")
            print(f"#### Initial number of intervals: {self.__format_verbose(n_rad_np)}")
            print("#############################################################################")
