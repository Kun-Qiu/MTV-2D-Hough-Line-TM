...
def create_centered_grid(image_size, fwhm, spacing, angle, line_intensity=0.5, num_lines=10, snr=20):
    ...
    angle1 = intersection_angle/2 if angle != 90 else np.pi/2   # First line angle 
    angle2 = -intersection_angle/2 if angle != 90 else 0        # Second line angle 
    ...
    # Draw second set of parallel lines (at angle2)
    for i in range(-num_lines // 2, num_lines // 2 + 1):
